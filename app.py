...
with t1:
    if df_perf is not None:
        ...
        st.info("Upload Performance Data in sidebar.")
...
with t2:
    if df_audit is not None:
        ...
        if ag_col and cs_col:
            ...
            st.error("Audit file must have 'Agent' and 'Csat' columns.")
    else:
        st.info("Upload Audit Data in sidebar.")
...
with t3:
    ...
    c1, c2 = st.columns(2)
    if df_perf is not None: 
        with c1: st.write("Perf Data"), st.dataframe(df_perf)
    if df_audit is not None: 
